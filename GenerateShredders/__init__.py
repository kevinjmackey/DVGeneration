        ...
        for entity in entities:
            if entity.stageColumnName.endswith("RECORD_KEY") or entity.stageColumnName in fieldsToSkip:
                ...
                if entity.stageTableName.endswith("_REOCCUR"):
                    ...
                    reoccurDataTypes[entity.stageColumnName] = "varchar(510)" if entity.stageEasyDataType.upper().startswith("VARCHAR(2") else entity.stageEasyDataType.upper()
                else:
                    baseColumns.append(entity.stageColumnName)
                    baseDataTypes[entity.stageColumnName] = "varchar(510)" if entity.stageEasyDataType.upper().startswith("VARCHAR(2") else entity.stageEasyDataType.upper()
...
async def main(req: func.HttpRequest, inputBlob: func.InputStream, context: func.Context) -> func.HttpResponse:
    log.info(f'XML Shredders function processed a request: {req.get_json()}')
    ...
    for source in sources.keys():
        if str(source) not in exclude:
            try:
                ...
                outputFile = f"{filePath}{filePrefix}XMLShredder{str(source)}.cs"
                shredder = (t(**d))
                block_blob_service.create_blob_from_text(container, outputFile, shredder, encoding='utf-8')
